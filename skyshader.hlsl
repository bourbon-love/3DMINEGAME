...
cbuffer SkyConstantBuffer : register(b0)
{
    ...
    float4 TopColor; // 顶部颜色
    float4 MiddleColor; // 中间颜色
    float4 BottomColor; // 底部颜色
    float3 SunPosition; // 添加太阳位置
    float Time; // 添加时间参数
...
float4 skyPS(VS_OUTPUT input) : SV_Target
{
    ...
    // 向最终颜色添加太阳光晕
    skyColor.rgb += float3(1.0f, 0.9f, 0.7f) * sunIntensity;
