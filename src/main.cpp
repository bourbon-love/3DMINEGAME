	...
	HINSTANCE hPrevInstance, LPSTR lpCmd, int nCmdShow)
{
	...
	{	//メッセージの有無をチェック
		...
		else //Windowsからメッセージが来ていない
		{
			...
			if ((dwCurrentTime - dwExecLastTime) >= ((float)1000 / 60)) // 1/60秒经过了吗
			{
				dwExecLastTime = dwCurrentTime;	// 保存当前时间
#ifdef _DEBUG
				wsprintf(g_DebugStr, WINDOW_CAPTION);// 数组中存入标题文字
				wsprintf(&g_DebugStr[strlen(g_DebugStr)], " FPS:%d", g_CountFPS);
				SetWindowText(hWnd, g_DebugStr);// 更改标题部分
#endif
				::ShowCursor(TRUE);
				Update();	// 更新处理
				Draw();		// 绘制处理
...
/*
	...
	Begin();	//デフォルトのレンダリングターゲットへ切り替え

	//ここで表示した物はバックバッファへ描かれる=========
	...
	GetDeviceContext()->PSSetShaderResources(0, 1, &ptexture);//レンダリングテクスチャセット

	XMMATRIX	WorldMatrix;
	WorldMatrix = XMMatrixTranslation(SCREEN_WIDTH/2, SCREEN_HEIGHT/2,0);
	//ワールド行列をDirectXへセット
	SetWorldMatrix(WorldMatrix);
	XMFLOAT2 size = XMFLOAT2(SCREEN_WIDTH/1, SCREEN_HEIGHT/1);
	XMFLOAT4 color = XMFLOAT4(1, 1, 1, 1);
	DrawSprite( size,  color);	//画面サイズのスプライト表示
